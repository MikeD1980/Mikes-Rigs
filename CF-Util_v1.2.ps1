...
# Warning message
$warningMessage = "WARNING: This test is destructive and will erase all files on $driveLetter drive. Do you want to continue? (Y/N)"
...
#	cls
    ...
    Write-Host "WARNING: This test is destructive and will erase all data on $DriveLetter drive."
...
function Option4 {
    ...
    # Warning message
    $warningMessage = "WARNING: This operation is destructive and will erase all files on $driveLetter drive. Do you want to continue? (Y/N)"
    $continue = Read-Host $warningMessage
...
foreach ($Partition in Get-Partition -DiskNumber $SelectedDisk.Number) {
    ...
    if ($Alignment -eq 0) {
        ...
        Write-Log " Partition $($Partition.PartitionNumber) is NOT aligned. Offset: $Offset"
    }
}
...
function Restore-CFCard {
    ...
    Write-Host "========================================================================" -ForegroundColor Green
	Write-Host "NOTE: You can highligh file names with the mouse, press CTRL+C to copy"
	Write-Host "      then just press CTRL+V to paste in the file name."
    $backupDir = "C:\CFFiles"
    $backupFiles = Get-ChildItem -Path $backupDir -Filter *.CFC
    $backupFiles | ForEach-Object { Write-Host $_.Name }
    $backupFile = Read-Host "Enter one of the following backup file names to restore from or press Enter to abort -"
    $logFile = "C:\CFLog.txt"
...
function View-BackupFiles {
    ...
    $backupFiles | ForEach-Object { Write-Host $_.Name }
	Write-Host ""
    Write-Host "NOTE - You can highlight the file name with the mouse and press CTRL+C"
	Write-Host "to copy the file name to use for the Restore function."
	Write-Host ""
	Pause
}
